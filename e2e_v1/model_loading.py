...
def _has_all_files(root: Path, filenames: List[str]) -> bool:
    return all((root / filename).exists() for filename in filenames)
...
def _resolve_nllb_tokenizer_source(nllb_path: str) -> str:
    ...
    has_slow_tokenizer = _has_all_files(
        path,
        ["sentencepiece.bpe.model", "tokenizer_config.json"],
    )
    has_fast_tokenizer = _has_all_files(
        path,
        ["tokenizer.json", "tokenizer_config.json"],
    )
    if has_slow_tokenizer or has_fast_tokenizer:
        return nllb_path
...
def _apply_decoder_lora(nllb_model):
    if LoraConfig is None or TaskType is None or get_peft_model is None:
        raise ImportError(
            "Decoder LoRA was requested, but 'peft' is not installed. "
            "Add peft to the environment before using --unfreeze_decoder_lora."
        )
    ...
    model = get_peft_model(nllb_model, lora_config)
    print("[model_loading] Applied decoder LoRA adapters to NLLB.")
    return model
...
def build_processor_and_tokenizer(
    ...
    tokenizer = None
    tokenizer_errors = []
    candidate_sources = [tokenizer_source]
    if tokenizer_source != NLLB_HUB:
        candidate_sources.append(NLLB_HUB)
    ...
    for candidate in candidate_sources:
        try:
            ...
            tokenizer_errors.append(f"{candidate} (fast): {exc}")
            print(
                "[model_loading] Fast tokenizer load failed for NLLB; "
                f"source={candidate}; error={exc}"
            )
        ...
        try:
            ...
            tokenizer_errors.append(f"{candidate} (slow): {exc}")
            print(
                "[model_loading] Slow tokenizer load failed for NLLB; "
                f"source={candidate}; error={exc}"
            )

    if tokenizer is None:
        raise OSError(
            "Unable to load an NLLB tokenizer from any candidate source. "
...
class MAdapter(nn.Module):
    """
    A stronger modality adapter for mapping speech encoder features into the
    text-decoder space.

    Architecture:
      1. Temporal downsampling with the same strided-conv front-end used by the
         baseline adapter.
      2. Projection into the NLLB decoder hidden size.
      3. A small stack of TransformerEncoder layers operating in decoder space.
    """
    ...
    def __init__(
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=dec_dim,
            nhead=num_heads,
            dim_feedforward=dec_dim * ff_mult,
            dropout=dropout,
            batch_first=True,
            norm_first=True,
            activation="gelu",
    ...
    def _downsample_mask(
        ...
        return (mask_f.squeeze(1) > 0).long()
    ...
    def forward(
        ...
        adapted_mask = self._downsample_mask(attention_mask)
        key_padding_mask = None
        if adapted_mask is not None:
            key_padding_mask = adapted_mask == 0
...
class InterConnectionST(nn.Module):
    ...
    def __init__(
        ...
        aux_special_token_ids: Optional[List[int]] = None,
        ...
        vocab_size     = self.nllb.config.vocab_size

        self.aggregator    = LayerAggregator(aggregation_layers, num_enc_layers)
        if self.aux_ctc_weight > 0:
            self.aux_ctc_head = nn.Linear(enc_dim, vocab_size)
            self.aux_ctc_loss_fn = nn.CTCLoss(
                blank=getattr(self.nllb.config, "pad_token_id", 1),
                zero_infinity=True,
            )
            ignore_ids = sorted(
                set((aux_special_token_ids or []) + [getattr(self.nllb.config, "pad_token_id", 1)])
            )
            self.register_buffer(
                "aux_ignore_token_ids",
                torch.tensor(ignore_ids, dtype=torch.long),
                persistent=False,
            )
        else:
            self.aux_ctc_head = None
            self.aux_ctc_loss_fn = None
            self.register_buffer(
                "aux_ignore_token_ids",
                torch.empty(0, dtype=torch.long),
                persistent=False,
            )
        if adapter_type == "m_adapter":
            self.adapter = MAdapter(
                enc_dim=enc_dim,
                dec_dim=dec_dim,
                stride=adapter_stride,
                num_conv_layers=adapter_num_convs,
                num_layers=m_adapter_layers,
                num_heads=m_adapter_heads,
                dropout=m_adapter_dropout,
            )
            print(
                "[model_loading] Using M-Adapter bridge "
                f"(layers={m_adapter_layers}, heads={m_adapter_heads}, stride={adapter_stride}, "
                f"conv_layers={adapter_num_convs})."
            )
        else:
            self.adapter = LengthAdapter(enc_dim, dec_dim, adapter_stride, adapter_num_convs)
            print(
                "[model_loading] Using LengthAdapter bridge "
                f"(stride={adapter_stride}, conv_layers={adapter_num_convs})."
            )
        self.length_adapter = self.adapter
        if self.aux_ctc_weight > 0:
            print(f"[model_loading] Auxiliary target-side CTC loss active (weight={self.aux_ctc_weight:.2f}).")
        else:
            print("[model_loading] Auxiliary target-side CTC loss disabled.")
    ...
    def set_decoder_lora_trainable(self, trainable: bool = True):
        found = False
        for name, param in self.nllb.named_parameters():
            if "lora_" in name:
                param.requires_grad = trainable
                found = True
        if found:
            state = "trainable" if trainable else "frozen"
            print(f"[model_loading] Decoder LoRA parameters set to {state}.")
        else:
            print("[model_loading] No decoder LoRA parameters were found.")
    ...
    def _get_decoder_layers(self):
        model = self._get_base_nllb_model()
    ...
    def _get_base_nllb_model(self):
        if hasattr(self.nllb, "get_base_model"):
            return self.nllb.get_base_model()
        return self.nllb
    ...
    def _prepare_aux_ctc_targets(self, labels: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        targets = []
        target_lengths = []
        ignore_ids = self.aux_ignore_token_ids.to(labels.device)

        for row in labels:
            valid = row[row != -100]
            if ignore_ids.numel() > 0 and valid.numel() > 0:
                valid = valid[~torch.isin(valid, ignore_ids)]
            if valid.numel() == 0:
                valid = row[row != -100]
            targets.append(valid)
            target_lengths.append(int(valid.numel()))

        if not targets:
            empty = torch.empty(0, dtype=torch.long, device=labels.device)
            return empty, torch.empty(0, dtype=torch.long, device=labels.device)

        return (
            torch.cat(targets),
            torch.tensor(target_lengths, dtype=torch.long, device=labels.device),
        )

    def _compute_aux_ctc_loss(
        self,
        aggregated: torch.Tensor,
        enc_mask: Optional[torch.Tensor],
        labels: torch.Tensor,
    ) -> torch.Tensor:
        if self.aux_ctc_head is None or self.aux_ctc_loss_fn is None:
            return aggregated.new_zeros(())

        logits = self.aux_ctc_head(aggregated.float())
        log_probs = F.log_softmax(logits, dim=-1).transpose(0, 1)

        if enc_mask is not None:
            input_lengths = enc_mask.long().sum(dim=-1)
        else:
        ...
        targets, target_lengths = self._prepare_aux_ctc_targets(labels)
        if targets.numel() == 0:
            return aggregated.new_zeros(())

        return self.aux_ctc_loss_fn(
            log_probs,
            targets,
            input_lengths,
            target_lengths,
    ...
    def forward(
        ...
        adapted, ada_mask = self.adapter(aggregated, enc_mask)
        ...
        if labels is not None and self.aux_ctc_weight > 0:
            aux_ctc_loss = self._compute_aux_ctc_loss(aggregated, enc_mask, labels)
            outputs.loss = outputs.loss + self.aux_ctc_weight * aux_ctc_loss
            outputs.aux_ctc_loss = aux_ctc_loss.detach()
        return outputs
...
def build_model(
    ...
    Args:
        ...
        adapter_type       : "length" baseline or "m_adapter" stronger bridge
        m_adapter_layers   : number of Transformer layers inside M-Adapter
        m_adapter_heads    : attention heads inside M-Adapter
        m_adapter_dropout  : dropout used inside M-Adapter
        aux_ctc_weight     : always-on auxiliary target-side CTC loss weight
        aux_special_token_ids : tokenizer special ids to ignore in the auxiliary CTC targets
        freeze_encoder     : freeze ASR encoder weights (recommended)
        freeze_decoder     : freeze NLLB decoder weights to save GPU memory
        decoder_lora       : attach trainable LoRA adapters to the decoder
    """
    asr_source, asr_kind = _resolve_asr_source(asr_url, cache_dir)
    nllb_path = _resolve_model_path(nllb_url,     cache_dir, "nllb")
    ...
    model = InterConnectionST(
        ...
        aux_special_token_ids=aux_special_token_ids,
