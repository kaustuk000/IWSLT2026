...
DEFAULT_DATA_DIR = "./IWSLT2026/iwslt2026_bho_hi"
...
def _processor_input_name(processor) -> str:
    return getattr(processor, "_asr_input_name", "input_values")
...
class BhoHinSTDataset(Dataset):
    ...
    Args:
        ...
        processor         : ASR processor (Wav2Vec2 or Whisper)
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # ---- encoder features ----
        input_name = _processor_input_name(self.processor)
        processor_kwargs = {
        ...
        encoder_inputs = getattr(enc, input_name)[0]
        attention_mask = (
        ...
        if self.augment and random.random() < 0.5:
            encoder_inputs = spec_augment(encoder_inputs)
...
class SpeechTranslationCollator:
    ...
    def __call__(self, features: List[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if sample.ndim == 1:
            encoder_inputs = np.zeros((len(features), max_aud), dtype=np.float32)
            for i, f in enumerate(features):
                l = input_lengths[i]
                encoder_inputs[i, :l] = f["encoder_inputs"]
        elif sample.ndim == 2:
            feature_dim = sample.shape[0]
            encoder_inputs = np.zeros((len(features), feature_dim, max_aud), dtype=np.float32)
            for i, f in enumerate(features):
                l = input_lengths[i]
                encoder_inputs[i, :, :l] = f["encoder_inputs"][:, :l]
        else:
            raise ValueError(
                f"Unsupported encoder input rank: {sample.ndim}"
            )
